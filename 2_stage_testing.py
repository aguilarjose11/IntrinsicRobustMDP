...
from attackability import characterize_robustness_torch
...
if __name__ == "__main__":
    ...
    # Lunar Lander: 
    # env = gym.make("LunarLander-v3") # Make sure it is installed
    
    # Construct LinearMDP-learning 
    ...
    # Replace for learnt target policy from other training.
    ...
    stage_2_cutoff = 500 # episodes
    stage_2_flag = False
    stage_2_fail_override = True
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            next_obs, reward, terminated, truncated, info = env.step(np.argmax(action.detach().cpu().numpy()), learning=not stage_2_flag)
            done = terminated or truncated
            # Poisoning Stage
            with torch.no_grad():
                if episode <= stage_2_cutoff:
                    # Poison with AT
                    perturbation = at_attack.compute_attack(obs, action).item()
                elif not stage_2_flag: # Aka. Stage 2 set up
                    # Poison with Characterization
                    n_samples = 3_000
                    components = env.get_learned_components()
                    phi = components['phi']
                    theta_star = env.get_reward_params()
                    states, _, _, _, _ = env.replay_buffer.sample(n_samples, device=device)
                    relevant_states = torch.unique(states, dim=0)
                    M_pi_dagger = env.get_transition_params()
                    # TODO: Only for discrete envs
                    action_space = list(range(env.action_space.n)) # or self._get_flattened_dim(self.action_space)
                    print('Computing characterization...')
                    epsilon, theta_dagger, status = characterize_robustness_torch(phi, 
                                                                                  actor, 
                                                                                  theta_star, 
                                                                                  relevant_states, 
                                                                                  action_space, 
                                                                                  ...
                                                                                  device=device)
                    theta_dagger = torch.Tensor(theta_dagger).to(device)
                    stage_2_flag = True # Above done only once
                if stage_2_flag: # Stage 2 poisoning
                    if epsilon > 0. or stage_2_fail_override:
                        # conduct attack
                        z_phi = phi(obs, action)
                        r_hat = (z_phi @ theta_dagger).cpu().item()
                        perturbation = min(r_hat - reward, 0.) # Ensures no positive perts.
                        # reward_ = r_hat # TODO: add random noise eta
                    else:
                        print(f"Epsilon: {epsilon} <= 0 -- Robust Environment.")
                        break
